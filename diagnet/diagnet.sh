...
tcp_probe(){ local host="$1" port="$2"; local t0; t0=$(date +%s%N); if timeout 2 bash -c "exec 3<>/dev/tcp/${host}/${port}" 2>/dev/null; then exec 3>&-; local ms=$(( ( $(date +%s%N) - t0 ) / 1000000 )); log "TCP_OK target=${host}:${port} connect_ms=${ms}"; if (( ms > TCP_THRESH_MS )); then if guard "TCP_${host}_${port}"; then log "TCP_SLOW_TRIGGER target=${host}:${port} connect_ms=${ms}"; trigger "TCP_SLOW ${host}:${port}" "$host" "$port"; fi; fi; else local ms=$(( ( $(date +%s%N) - t0 ) / 1000000 )); log "TCP_FAIL target=${host}:${port} connect_ms=${ms}"; if guard "TCP_${host}_${port}"; then trigger "TCP_FAIL ${host}:${port}" "$host" "$port"; fi; fi; }
echo_rtt(){ local host="$1" port="$2"; if ! command -v python3 >/dev/null 2>&1; then log "ECHO_SKIP target=${host}:${port} reason=python3_not_found"; return 0; fi; local out rc; set +e; out=$(python3 - "$host" "$port" <<'PY'
