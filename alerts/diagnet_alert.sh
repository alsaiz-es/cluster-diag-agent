...
summarize(){ local label="$1"; local data="$2"; echo "### ${label}"; echo "$data" | grep -E "(_SLOW_TRIGGER|_FAIL|SCHED_JITTER|NET_FAIL)" || true; echo; echo "Summary ${label}:"; echo "  PING_FAIL   : $(echo "$data" | grep -c 'PING_FAIL' || true)"; echo "  PING_SLOW   : $(echo "$data" | grep -c 'PING_SLOW_TRIGGER' || true)"; echo "  TCP_FAIL    : $(echo "$data" | grep -c 'TCP_FAIL' || true)"; echo "  TCP_SLOW    : $(echo "$data" | grep -c 'TCP_SLOW_TRIGGER' || true)"; echo "  ECHO_FAIL   : $(echo "$data" | grep -c 'ECHO_FAIL' || true)"; echo "  ECHO_SLOW   : $(echo "$data" | grep -c 'ECHO_SLOW_TRIGGER' || true)"; echo "  JITTER      : $(echo "$data" | grep -c 'SCHED_JITTER' || true)"; echo "  NET_FAIL    : $(echo "$data" | grep -c 'NET_FAIL' || true)"; echo "  DNS_FAIL    : $(echo "$data" | grep -c 'DNS_FAIL' || true)"; echo "  DNS_SLOW    : $(echo "$data" | grep -c 'DNS_SLOW_TRIGGER' || true)"; echo; }
...
{ echo "==== DiagNet/VmWatch ALERT summary (UTC) ===="; echo "Window: $SINCE .. $NOW"; echo; [[ -n "$diag" ]] && summarize "DiagNet" "$diag" || { echo "No DiagNet data in window."; echo; }; [[ -n "$vmw" ]] && summarize "VmWatch" "$vmw" || { echo "No VmWatch data in window."; echo; }; echo "New artifacts: snapshots=$NEW_SNAP pcaps=$NEW_PCAP sar1s=$NEW_SAR"; echo "============================================="; echo; } | tee -a "$OUT"
INC=$(( $(echo "$diag$vmw" | grep -E "(_SLOW_TRIGGER|_FAIL|SCHED_JITTER|NET_FAIL|DNS_FAIL)" -c || true) ))
if command -v logger >/dev/null 2>&1; then
  if (( INC > 0 )); then logger -p user.warning "DiagNet/VmWatch: ${INC} incidents in ${WINDOW} min (see $OUT)"; else logger -p user.info "DiagNet/VmWatch: no incidents in ${WINDOW} min"; fi
fi
if (( INC > 0 )) && command -v mail >/dev/null 2>&1; then mail -s "ALERT DiagNet/VmWatch (${INC} incidents)" root@localhost < "$OUT" || true; fi
exit $(( INC > 0 ? 2 : 0 ))
