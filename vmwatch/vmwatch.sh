...
now_ns(){ date +%s%N; }
net_check(){ local peer="$1"; local host="${peer%%:*}"; local port="${peer##*:}"; local sns; sns=$(now_ns); if timeout 2 bash -c "exec 3<>/dev/tcp/${host}/${port}" 2>/dev/null; then exec 3>&-; local ens; ens=$(now_ns); local ms=$(( (ens - sns) / 1000000 )); log "NET_OK peer=${peer} connect_ms=${ms}"; else local ens; ens=$(now_ns); local ms=$(( (ens - sns) / 1000000 )); log "NET_FAIL peer=${peer} connect_ms=${ms}"; fi; }
